...
// per-symbol historical OHLC table — populated by the IBKR backfill, FRED
// backfill, or whatever else fed it. The reported `source` is the actual
// `source` column of the latest row, not a hardcoded label.
export async function loadQuoteFromHistory(pool, sym) {
  const closeCol = sym.section === 'rates' ? '"yield"' : 'close';
  let rows;
  try {
    const r = await pool.query(
      `SELECT date, ${closeCol} AS close, source FROM ${sym.schema_name}.${sym.table_name}
  ...
  return {
    ...
    source: last.source || 'unknown', ts: last.date,
  };
}
